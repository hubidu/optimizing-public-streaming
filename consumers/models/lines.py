...
class Lines:
    ...
    def process_message(self, message):
        ...
        if "cta.db.stations_transformed" in message.topic():
            value = message.value()
            if message.topic() == "cta.db.stations_transformed.table.v1":
        ...
        elif "cta.turnstiles-summary" == message.topic() or "cta.arrivals" in message.topic():
            self.green_line.process_message(message)
            self.red_line.process_message(message)
            self.blue_line.process_message(message)
        else:
